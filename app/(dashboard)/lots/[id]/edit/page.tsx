...
export default async function EditLotPage({
  ...
  return (
    <div className="max-w-2xl space-y-5">
      <div>
        <Link href={`/lots/${id}`} className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-400 hover:text-zinc-700 transition-colors mb-4">
          <ArrowLeft size={13} />
          Lote {lot.lot_number}
        </Link>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-zinc-900">Editar lote</h2>
            <p className="text-xs text-zinc-500 mt-0.5">{lot.products.name} — {lot.products.line}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
        ...
        <form action={action} className="px-6 py-5 space-y-4">
          ...
          <div className="pt-1 flex gap-3">
            ...
            <Link href={`/lots/${id}`} className="flex-1 text-center border border-zinc-300 hover:bg-zinc-50 text-zinc-700 font-medium rounded-xl py-2.5 text-sm transition-colors">
              Cancelar
            </Link>
          </div>
        </form>
